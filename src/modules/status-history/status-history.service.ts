...
type TaskStatusHistoryWithUser = Prisma.TaskStatusHistoryGetPayload<{
  include: {
    changedBy: {
      select: {
        id: true;
...
export class StatusHistoryService {
  ...
  async findByTaskId(
    ...
    const whereCondition: Prisma.TaskStatusHistoryWhereInput = {
      taskId,
    ...
    const historyWithDuration = this.calculateDurationBetweenStatuses(history);
  ...
  async getTaskSummary(taskId: string, userId: string) {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (nextChange) {
        const duration =
          nextChange.changedAt.getTime() - change.changedAt.getTime();
   ...
   */
  private calculateDurationBetweenStatuses(
    history: TaskStatusHistoryWithUser[],
  ): HistoryItem[] {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (next) {
        durationInStatusMs =
          current.changedAt.getTime() - next.changedAt.getTime();
      }
      ...
      const historyItem: HistoryItem = {
        ...
        durationInStatus: durationInStatusMs
          ? this.formatDuration(durationInStatusMs)
          : null,
