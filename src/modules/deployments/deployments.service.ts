...
export class DeploymentsService {
    ...
    async findOne(deploymentId: string, userId: string) {
        ...
        if (!deployment) {
            throw new NotFoundException('Deployment not found or you do not have access');
        }

        return deployment;
    }

    async updateStatus(deploymentId: string, userId: string, dto: UpdateDeploymentStatusDto) {
        ...
        if (!isProjectAdmin && !isDeployer) {
            throw new ForbiddenException('Only project admin or deployment executor can update status');
        }
        ...
        await this.sendDeploymentNotification(deploymentId, dto.status.toLowerCase());
    ...
    async remove(deploymentId: string, userId: string) {
        ...
        if (!isProjectAdmin && !isWorkspaceAdmin) {
            throw new ForbiddenException('Only project admin or workspace admin can delete deployments');
        }
    ...
    private async processDeployment(deploymentId: string, userId: string) {
        ...
        await this.updateStatus(deploymentId, userId, { status: DeploymentStatus.RUNNING });

        await this.delay(3000); // Simulate deployment
        await this.updateStatus(deploymentId, userId, { status: DeploymentStatus.SUCCESS });
    }

    private async sendDeploymentNotification(deploymentId: string, status: string) {
        try {
            const deployment = await this.prisma.deployment.findUnique({
                where: { id: deploymentId },
                include: {
            ...
            const taskList = deployment.tasks.map(t => `  • ${t.task.title}`).join('\n');
