...
export class StatusHistoryController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns status history for the task',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns statistics about status changes',
  })
  getTaskSummary(
    @Param('taskId') taskId: string,
    @GetUser('id') userId: string,
  ...
  getWorkspaceActivity(
    ...
    return this.statusHistoryService.getWorkspaceActivity(
      workspaceId,
      userId,
      limit || 50,
    );
